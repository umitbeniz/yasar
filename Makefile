...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=yasar \
		  -X github.com/cosmos/cosmos-sdk/version.AppName=yasard \
...
build: go.sum
ifeq ($(OS),Windows_NT)
	go build -mod=readonly $(BUILD_FLAGS) -o build/yasard.exe ./cmd/yasard
else
	go build -mod=readonly $(BUILD_FLAGS) -o build/yasard ./cmd/yasard
endif
...
install: go.sum
	go install -mod=readonly $(BUILD_FLAGS) ./cmd/yasard
